...
质控不是为了把数据“洗干净”到好看，而是为了尽量保留真实生物信号，同时减少低质量细胞、空液滴、doublet 和技术噪声对结论的影响。
...
Scanpy 官方教程示例中会使用 Scrublet 思路进行 doublet 检测。实践上建议把 doublet 分数作为证据之一，而不是唯一判决。
...
<div class="doc-callout warning">
  <strong>不要把 doublet 和低质量细胞混成一类</strong>
  <p>doublet 常表现为两个细胞类型 marker 同时偏高；低质量细胞则常伴随低基因数、高线粒体比例或异常 total counts。两者排查逻辑不同。</p>
</div>
...
官方基础流程使用 total-count normalization 加 `log1p`，再选择 highly variable genes。它适合入门和许多常规探索，但大队列、强批次、多样本整合时，可能要考虑更系统的归一化和整合策略。
...
回归掉 total counts 或线粒体比例在教程中常见，但不应无脑使用。因为这些技术指标有时和真实细胞状态相关，过度回归可能擦掉生物信号。
